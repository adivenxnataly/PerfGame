...
package=$(pm list packages -3 | grep -E 'com.mobile\.|com.miHoYo\.|com.HoYoverse\.|com.kurogame\.' | sed 's/package://g' | sed 's/^/   /'); sleep 2
ui_print "   List :"
sleep 1
ui_print "$package"
ui_print ""
awk '
    /const gameList = \[/ {
        ...
        while ("pm list packages -3" | getline package) {
            if (package ~ /com\.mobile\.|com\.miHoYo\.|com\.HoYoverse\.|com\.kurogame\./) {
                split(package, arr, ":")
                print "            \"" arr[2] "\","
            }
        }
