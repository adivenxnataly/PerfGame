document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadGameList() {
        ...
        if (gameList.length === 0) {
            ...
            currentModeButton.classList.add('hidden');
            ...
            gameList.forEach(game => {
                executeCommand(`cmd game mode performance ${game}`);
        ...
        getResolutionHeight(packageName,
            function(resolutionHeight) {
                if (resolutionHeight) {
                    ...
                    resolutionInfo.textContent = 'Resolution Not found';
                }
            });
    ...
    function applyConfiguration(packageName) {
        ...
        executeCommand(configCommand,
            function(output) {
                ...
                checkConfiguration(packageName);
            });
    }
    ...
    function setMode(mode) {
        ...
        modeIndicator.classList.remove('bg-red-600', 'bg-green-400', 'bg-orange-600');
        ...
        if (mode === 'Performance') {
            ...
            modeIndicator.classList.add('bg-orange-600');
        }
    ...
    helpButton.addEventListener('click',
        function(event) {
            event.preventDefault();
            popupMessage.textContent = "This General Mode is the mode used by Games (that support it), Game Mode is provided by Android (API 31).";
    ...
    function executeCommand(command, callback) {
        ...
        try {
            ...
            console.error('Error executing ksu.exec:', error);
            if (callback) callback(null);
        }
    }
});